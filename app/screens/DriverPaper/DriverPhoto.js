...
const DriverPhoto = ({ navigation }) => {
  ...
  const [avatar, setAvatar] = useState(userD?.driverPhoto);

  const updateAvatarUser = async (downloadURL) => {
    const userRef = doc(db, "users", userDocId);
    await updateDoc(userRef, {
      driverPhoto: downloadURL,
      updatedAt: new Date(),
    })
      .then(() => {
  ...
  const handleUpload = async () => {
    if (avatar.length > 0) {
      ...
      const storageRef = ref(
        storage,
        `${userD?.email}/driverPhoto/${url_uuid}.png`
      );
      try {
      ...
      <ScrollView style={styles.scrollView}>
        ...
        <View style={styles.pictureContainer}>
          ...
          >
            ...
            <Text style={styles.editBtnTitle}>Edit Driver Photo</Text>
          </TouchableOpacity>
        </View>
